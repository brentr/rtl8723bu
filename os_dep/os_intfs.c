...
static int rtw_ndev_init(struct net_device *dev)
{
	...
	DBG_871X_LEVEL(_drv_info_, FUNC_ADPT_FMT"\n", FUNC_ADPT_ARG(adapter));
...
void rtw_dev_unload(PADAPTER padapter)
{
	...
	if (padapter->bup == _TRUE)
	{
		...
		if(rtw_hal_check_ips_status(padapter) == _TRUE || pwrctl->rf_pwrstate == rf_off) { //check HW status and SW state
			DBG_871X_LEVEL(_drv_notice_, "%s: driver in IPS-FWLPS\n", __func__);
			pdbgpriv->dbg_dev_unload_inIPS_cnt++;
			LeaveAllPowerSaveMode(padapter);
		} else {
			DBG_871X_LEVEL(_drv_notice_, "%s: driver not in IPS\n", __func__);
		}
		...
		if (padapter->bSurpriseRemoved == _FALSE)
		{
			...
			if (pwrctl->bSupportRemoteWakeup == _TRUE &&
				...
				DBG_871X_LEVEL(_drv_notice_, "%s bSupportRemoteWakeup==_TRUE  do not run rtw_hal_deinit()\n",__FUNCTION__);
			}
			else
#endif
			{
...
static int rtw_suspend_free_assoc_resource(_adapter *padapter)
{
	...
	{
		DBG_871X_LEVEL(_drv_notice_, "%s: fw_under_linking\n", __FUNCTION__);
		rtw_indicate_disconnect(padapter);
	}
...
int rtw_suspend_wow(_adapter *padapter)
{
	...
	if (pwrpriv->wowlan_mode == _TRUE) {
		...
		DBG_871X_LEVEL(_drv_notice_, "%s: wowmode suspending\n", __func__);

		if (check_fwstate(pmlmepriv, _FW_UNDER_SURVEY) == _TRUE)
		{
			DBG_871X_LEVEL(_drv_notice_, "%s: fw_under_survey\n", __func__);
			rtw_indicate_scan_done(padapter, 1);
		...
		if(pwrpriv->wowlan_pno_enable)
			DBG_871X_LEVEL(_drv_notice_, "%s: pno: %d\n", __func__, pwrpriv->wowlan_pno_enable);
		#ifdef CONFIG_LPS
		else
	...
	{
		DBG_871X_LEVEL(_drv_notice_, "%s: ### ERROR ### wowlan_mode=%d\n", __FUNCTION__, pwrpriv->wowlan_mode);
	}
	DBG_871X("<== "FUNC_ADPT_FMT" exit....\n", FUNC_ADPT_ARG(padapter));
	return ret;
}
...
static int rtw_suspend_normal(_adapter *padapter)
{
	...
	{
		DBG_871X_LEVEL(_drv_notice_, "%s: ### ERROR #### driver in IPS ####ERROR###!!!\n", __FUNCTION__);
...
int rtw_suspend_common(_adapter *padapter)
{
	...
	DBG_871X_LEVEL(_drv_notice_, " suspend start\n");
	...
	DBG_871X_LEVEL(_drv_notice_, "rtw suspend success in %d ms\n",
		rtw_get_passing_time_ms(start_time));
...
int rtw_resume_process_wow(_adapter *padapter)
{
	...
	if (pwrpriv->wowlan_mode == _TRUE) {
		...
		DBG_871X_LEVEL(_drv_notice_, "do not reset timer\n");
	}
...
int rtw_resume_common(_adapter *padapter)
{
	...
	DBG_871X_LEVEL(_drv_notice_, "resume start\n");
	DBG_871X("==> %s (%s:%d)\n",__FUNCTION__, current->comm, current->pid);
	...
	DBG_871X_LEVEL(_drv_info_, "%s:%d in %d ms\n", __FUNCTION__ ,ret,
		rtw_get_passing_time_ms(start_time));
