	...
	if (sta) {
		...
		if((mlmeinfo->state&0x03) == WIFI_FW_AP_STATE) {
			if((macid == 1) || (macid > (NUM_STA - 4))) {
				DBG_871X_LEVEL(_drv_err_, FUNC_ADPT_FMT
					       " failed, mac_id=%d\n",
		...
		if((((mlmeinfo->state&0x03) == WIFI_FW_AP_STATE) || ((mlmeinfo->state&0x03) == WIFI_FW_ADHOC_STATE))
			...
			if ((i = _rtw_camid_search(adapter, addr, kid)) >= 0) {
				...
				if (sta || _rtw_camid_is_gk(adapter, i) == _TRUE)
					...
					DBG_871X_LEVEL(_drv_warning_, FUNC_ADPT_FMT" group key id:%u the same key id as pairwise key\n"
						, FUNC_ADPT_ARG(adapter), kid);
				goto bitmap_handle;
			}
			...
			if (i == TOTAL_CAM_ENTRY) {
				if (sta)
					DBG_871X_LEVEL(_drv_warning_, FUNC_ADPT_FMT" pairwise key with "MAC_FMT" id:%u no room\n"
					, FUNC_ADPT_ARG(adapter), MAC_ARG(sta->hwaddr), kid);
				else
					DBG_871X_LEVEL(_drv_warning_, FUNC_ADPT_FMT" group key id:%u no room\n"
					, FUNC_ADPT_ARG(adapter), kid);
				rtw_warn_on(1);
				goto bitmap_handle;
			}
