	...
	if(prxattrib->encrypt==_TKIP_){
		...
		if (stainfo!=NULL){
			...
			if(IS_MCAST(prxattrib->ra))
			{
				...
				if(psecuritypriv->binstallGrpkey==_FALSE)
				{
					...
					if (rtw_get_passing_time_ms(start) > 1000) {
						if (no_gkey_bc_cnt || no_gkey_mc_cnt) {
							DBG_871X_LEVEL(_drv_notice_, FUNC_ADPT_FMT" no_gkey_bc_cnt:%u, no_gkey_mc_cnt:%u\n",
				...
				if (no_gkey_bc_cnt || no_gkey_mc_cnt) {
					DBG_871X_LEVEL(_drv_notice_, FUNC_ADPT_FMT" gkey installed. no_gkey_bc_cnt:%u, no_gkey_mc_cnt:%u\n",
						FUNC_ADPT_ARG(padapter), no_gkey_bc_cnt, no_gkey_mc_cnt);
				}
				start = 0;
				no_gkey_bc_cnt = 0;
...
u8 rtw_handle_tkip_countermeasure(_adapter* adapter, const char *caller)
{
	...
	if (securitypriv->btkip_countermeasure == _TRUE) {
		...
		if (passing_ms > 60*1000) {
			DBG_871X_LEVEL(_drv_notice_, "%s("ADPT_FMT") countermeasure time:%ds > 60s \n",
